...
| `screenshot` | Save a PNG/JPEG of the viewport, full page (`--full-page` / `--full-length`), or element (`--ref`). |
...
1. **Retrieve** — DuckDuckGo (default 3 hits) or site-only BFS when the entity
   is a URL / `--website` is set
2. **Crawl + extract** — httpx-first (no browser fallback for speed)
3. **Rerank** — Embed passages and score against the entity name
4. **Synthesize** — Local answer LLM emits JSON categories → Unicode tree;
   heuristic fallback via structured field extraction when `--no-llm`

**Output:** `KnowledgeGraph` with nested `nodes` and optional `sources`
