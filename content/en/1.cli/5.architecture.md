...
| `screenshot` | Save a PNG/JPEG of the viewport, full page (`--full-page` / `--full-length`), or element (`--ref`). Optional `--delay` waits before capture. |
...
- `graph.py` - Knowledge graph synthesis (`omniscout graph`)
...
#### Graph Engine (`graph.py` + `graph_facets.py`)

Builds structured entity knowledge graphs:

1. **Retrieve** — DuckDuckGo (default 5 hits) or site seed URLs when the entity
   is a URL / `--website` is set (`/`, `/pricing`, `/about`, `/team`)
2. **Crawl + extract** — httpx-first (no browser fallback for speed)
3. **Facet extraction** — Page-type routing (pricing from `/pricing`, founders
   from `/about`) and leaf sanitization (drop nav junk, marketing taglines)
4. **Optional LLM** — Classic local LLM overlay when `--llm` is set

**Output:** `KnowledgeGraph` with nested `nodes`, `retrieval` like `evidence+site`,
and optional `sources`

#### Structured Schema (`structured_schema.py`)

Exa-style JSON Schema extraction for `search --output-schema` and
`extract --schema`:

1. **Seed URLs** — DDG hits and/or inferred `{brand}.com/pricing` when the query
   names a product
2. **Multi-source merge** — NLP field extraction per page, linked-page enrichment
3. **Validate** — Required fields and type checks against the schema

**Output:** `{ "output": { "content": {...} }, "valid", "method": "heuristic" }`
