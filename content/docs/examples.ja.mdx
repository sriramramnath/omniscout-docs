---
title: 実例とレシピ
description: OmniScoutの実際のワークフロー集。
icon: FlaskConical
---

よくあるタスク向けのコピー＆ペースト用レシピです。背景の概念：[クイックスタート](/docs/quickstart)、[ブラウザ自動化](/docs/automation/browser)、[抽出](/docs/search/extraction)。
...
snapshot出力から検索ボックスのrefを探します — `@e`番号を推測しません。（`fill`は置換、`type`は追記、`paste`はカーソル位置に挿入します。）
