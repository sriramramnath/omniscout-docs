...
OmniScout now provides a `computer` sub‑command for macOS desktop automation. It offers high‑level actions that agents can invoke via the same JSON protocol.

### Core commands

| Command | Syntax | Description |
|---|---|---|
| `navigate` | `omniscout computer navigate <target> [--session <id>]` | Opens a file, URL, or application; sets the active app for the session. |
| `type` | `omniscout computer type <text> [--session <id>]` | Sends keystrokes to the active app, refocusing it first. |
| `key` | `omniscout computer key <combo> [--session <id>]` | Sends a keyboard shortcut (e.g., `cmd+n`). |
| `screenshot` | `omniscout computer screenshot [--out <path>] [--session <id>]` | Captures the screen via `screencapture`; returns file URL. |
| `clipboard get` | `omniscout computer clipboard get [--session <id>]` | Returns current clipboard contents. |
| `clipboard set` | `omniscout computer clipboard set <text> [--session <id>]` | Replaces the clipboard contents. |
| `window list` | `omniscout computer window list [--session <id>]` | Lists open windows (title, bundle ID). |
| `window activate` | `omniscout computer window activate <title|id> [--confirm] [--session <id>]` | Brings a window to the foreground. |
| `window close` | `omniscout computer window close <title|id> [--confirm] [--session <id>]` | Closes a window (requires `--confirm`). |
| `wait` | `omniscout computer wait <ms> [--session <id>]` | Simple sleep for the given milliseconds. |

For detailed usage, see the **Computer automation** reference page.
